...
```bash
node scripts/harness/selftest.mjs      # 11건, 0.3초
```

위반이 0 건인 검사는 **위반을 심어 보기 전까지 작동 여부를 모른다.** 검사가
망가져도 0 건은 그대로다.

```
fixtures/dirty/   판정마다 위반을 하나씩 심었다  → 누락 방향
fixtures/clean/   같은 구조에서 규칙을 지켰다     → 오탐 방향
selftest.mjs      기대값은 여기 한 곳에만
```

실제로 이 장치가 두 번 잡았다. `if (false && ...)` 로 판정 하나를 죽여 보면
`run.mjs` 는 6건을 잡아 **정상처럼 보이는데** selftest 만 `not ok 5` 를 낸다.

> **새 검사를 추가하면 표본도 추가한다.** 표본 없는 검사는 0 건이 정상인지
> 죽은 것인지 구별할 수가 없다.
...
단독 보도면 DB 쪽이 약 1.43배 높게 저장됐고, 그 값이 매일 찍히는 진영
스냅샷의 근거였다.

고치면서 검사도 한 번 고쳤다. 공식이 `score_core` 로 합쳐지자 예전 검사가
`calculate_score` 의 return 줄만 보고 오탐을 냈다. 지금은 `score_core` 를 보고
**공식 사본이 다시 생기는 것**까지 잡는다.

## 다음

1. 두 번째 검사 — 후보는 `constraints.md` 의 "아직 검사가 없는 것".
   가장 가까운 것은 사안 등급 규칙(Python ↔ TS)이다. M-01 과 같은 모양이다
2. `bias_audit` · `cross_verify` 배선 — 만들어놓고 안 도는 게 아직 둘 남았다
3. 0 건 검사의 회귀 확인 — 지금은 사람이 손으로 위반을 심어 보고 끝이라
   검사가 망가져도 0 건은 그대로다. charzing 세션도 같은 구멍을 안고 있다
