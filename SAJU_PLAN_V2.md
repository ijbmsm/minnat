...
| # | 항목 | 결정 |
|---|---|---|
| ① | 무료 | **크레딧 하나로 통일** (v4, 2026-09-17 재개정). 풀이 한 편 = 크레딧 1, KST 자정에 **1까지 top-up**(누적 안 됨). 화면 표기 `N/1`.<br>v3 는 "하루 한 편(불리언) + 획득 크레딧(숫자)" 두 개념이 따로 돌아 배지 문구가 네 갈래였다. v2 "계정당 1회"는 결제가 없어 막다른 길이 됐다 |
| ② | 다시 풀이받기 | 크레딧 1 소모 |
...
| ⑨ | 日 오늘의 사주 | 다른 타입과 동일하게 크레딧 1 소모. 매일 오는 사람은 자정 충전분을 오늘의 사주로 쓴다 |
| ⑩ | 궁합 초대 수락 | 크레딧을 **쓰지 않고** 양쪽 열람 + 양쪽 크레딧 +1. 두 사람을 데려오는 장치라 예외 |
...
### P2-6. 월 비용 킬스위치 ✅ 완료 — `src/lib/saju/spend.ts` (`SAJU_MONTHLY_CALL_LIMIT`, 기본 800 ≈ 월 $42 · 목표 DAU 20 기준, 실패 시 decr, 80% 경고), 503 `monthly_cap`
- **파일**: `src/lib/saju/credits.ts` 또는 신규 `src/lib/saju/spend.ts`, `reading/route.ts`·`compat/route.ts` LLM 호출 직전
- **변경**: Redis `saju:spend:{YYYYMM}` INCR (LLM 실제 호출 시만). `SAJU_MONTHLY_CALL_LIMIT` env(기본 800 — 2026-09-18 결정, 목표 DAU 20 × 하루 크레딧 1 × 30일 = 600 + 연속 보너스 + 여유. 여기 닿으면 사업자·PG 검토 시점). 초과 시 503 `{error:'monthly_cap'}` + UI "이번 달 준비된 풀이가 다 나갔어. 다음 달 1일에 다시 열려". 어드민 면제. 80% 도달 시 `console.warn` 1회.
