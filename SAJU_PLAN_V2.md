...
| # | 항목 | 결정 |
|---|---|---|
| ① | 무료 | 계정당 1회(타입 무관) + 획득형 크레딧(공유 가입·초대 수락·7일 연속) |
| ② | 다시 풀이받기 | 크레딧 1 소모 |
...
| ⑨ | 日 오늘의 사주 | **코멘트 반영: 매일 1회 무료.** 매일 오는 사용자를 위한 유일한 훅. 크레딧 미소모 |
...
### P2-6. 월 비용 킬스위치 ✅ 완료 — `src/lib/saju/spend.ts` (`SAJU_MONTHLY_CALL_LIMIT`, 기본 1000 = 월 약 $50, 실패 시 decr, 80% 경고), 503 `monthly_cap`
- **파일**: `src/lib/saju/credits.ts` 또는 신규 `src/lib/saju/spend.ts`, `reading/route.ts`·`compat/route.ts` LLM 호출 직전
- **변경**: Redis `saju:spend:{YYYYMM}` INCR (LLM 실제 호출 시만). `SAJU_MONTHLY_CALL_LIMIT` env(기본 1000 — 월 $50 상한 결정). 초과 시 503 `{error:'monthly_cap'}` + UI "이번 달 준비된 풀이가 다 나갔어. 다음 달 1일에 다시 열려". 어드민 면제. 80% 도달 시 `console.warn` 1회.
