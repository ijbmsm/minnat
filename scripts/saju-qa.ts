...
function check(sample: Sample): Finding[] {
  ...
  // (a)
  if (fs.bodyStrength === 'strong' && /인성[^。.\n]{0,12}(보강|보완|강화|채우|키우)/.test(text)) out.push({ file: sample.file, level: 'fail', rule: 'a.신강모순', detail: '신강 차트에 인성 보강 권장' });
  if (fs.bodyStrength === 'weak' && /(식상|식신|상관|재성)[^。.\n]{0,12}(활성|강화|키우|살리)/.test(text)) out.push({ file: sample.file, level: 'fail', rule: 'a.신약모순', detail: '신약 차트에 식상·재성 강화 권장' });
  ...
  for (const s of sample.sections) {
    if (TIMING_TITLE.test(s.title) && !/20\d\d년|\d{2}세|\d{2}대/.test(s.body)) out.push({ file: sample.file, level: 'warn', rule: 'd.연도없음', detail: `"${s.title}" 에 연도·나이 없음` });
  }
  return out;
}
