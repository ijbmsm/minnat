...
**3열 점수 요약 완전 삭제는 과하다:**
- 이슈별 점수는 있으니, "이 정치인과 관련된 이슈 N건, 총 점수 X" 정도는 보여줘도 됨
- **다만 "순점수" 같은 단일 숫자 평가는 삭제 동의**
- 제안: 이슈 목록만 보여주되, 각 이슈에 점수가 표시되어 있으므로 사용자가 스스로 판단
...
가장 큰 리스크: **"공식 처분만 점수"로 하면 데이터가 너무 적을 수 있음.** 형사 유죄 확정이 1년에 몇 건이나 되겠어. 이걸 보완하는 게 "진행 중 탭"과 "archive"인데, 사용자 입장에서 "점수가 거의 없는 스코어보드"가 매력적인지는 고민 필요.
