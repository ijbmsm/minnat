...
5. [본문 구조] 각 섹션 body 는 빈 줄로 나뉜 3문단, 공백 포함 240~320자.
   1문단(2~3문장): 결론부터. 두 사람 사이에서 그게 어떤 장면으로 나타나는지. 비유는 섹션당 최대 1개.
   2문단(2~3문장): 왜 그런지 근거. 두 사람의 일간·일지·십신·합충을 이름으로 짚어.
   3문단(2문장): 그래서 각자 뭘 하면 되는지. A 가 할 것 / B 가 할 것을 나눠서.
6. 구체적으로 — "잘 맞는다/안 맞는다" 뭉뚱그리기 금지. 어떤 상황에서 어떻게 나타나는지.
7. 일지충이면 갈등을 과장하지 말고 "이 시기 조급함 주의" 프레임으로.
8. 비유를 섹션마다 새로 만들지 말고, 같은 비유를 반복해서 쓰지도 마.`;
...
아래 6개 섹션으로, 라벨과 순서를 그대로 지켜서 분석해줘.

출력 형식: JSON 배열만. 앞뒤에 다른 말 붙이지 마.
[{"label":"...", "title":"...", "body":"..."}, ...]
- label: 아래 대괄호 라벨을 대괄호 없이 그대로 복사.
- title: 그 섹션을 한 줄로 찌르는 후킹 문장 (10~24자, 마침표 없이).
  첫 번째 섹션의 title 은 이 관계 전체를 한 줄로 정의하는 문장으로 써 — 화면 맨 위에 점수와 같이 걸린다.
  예) "제련되지 않은 원석과 뜨거운 용광로"
- body: 규칙 5의 3문단 구조. 문단 사이 줄바꿈은 JSON 문자열 안에서 반드시 \\n 으로 이스케이프해.

[케미] 일간·일지 관계로 보는 첫인상·끌림·기본 에너지 호환.
[속도] 두 사람의 리듬 차이 — 결정 속도, 감정 표현 속도, 일 처리 방식이 어디서 엇갈리는지.
[갈등] 마찰이 생기기 쉬운 구체적 상황과 그때 각자 할 수 있는 것.
[영향] A가 B를 어떻게 변화시키는지, B가 A에게 어떤 존재인지 (십신 역학 기반).
[시너지] 오행 보완으로 성과가 나는 영역. 이 조합이 실제로 뭘 잘 만들어내는지.
[오래가려면] 이 관계가 길어질 때 닳는 지점과, 그걸 막기 위해 미리 정해둘 규칙 1가지.`;
