...
기사 → 사건(`issue_clusters`) 위에 몇 년짜리 사안을 얹는다. **사람이 손대는 곳이 없다.**
발견·장 구분·등급은 규칙으로, 문장만 Haiku 가 쓴다. 자세한 것은
[`docs/storylines.md`](./docs/storylines.md).

- 등급 규칙이 Python(`storyline_shape.event_grade`)과 TS(`lib/story-format.deriveGrade`)
  양쪽에 있다. **한쪽만 고치면 화면과 DB 가 다른 말을 한다** — 테스트가 양쪽에 같은 표로 있다.
- LLM 에 날짜를 넘기지 않는다. 입력에 근거 없는 날짜가 나오면 원고 전체를 버린다.
- 저장소: `supabase/027-storylines-auto.sql`
...
1. **dedup 미흡** — 같은 사건이 3건씩 중복 저장됨.
   **원인 규명(2026-09-17)**: OpenAI 크레딧 소진으로 임베딩이 전부 NULL 이라
   Stage 2(임베딩 비교) 매칭이 몇 달간 꺼져 있었다.
   **임베딩 재생성 완료(2026-09-19)** — `backfill_embeddings.py --apply` 로 클러스터
   67건 전부 채웠다(NULL 67 → 0, 실패 0). Stage 2 가 다시 돈다.
   **남은 일**: 크롤러를 한 번 돌려 중복이 실제로 줄었는지 재평가. 그래도 남으면
   actor + category + 7일 이내 병합 규칙을 손봐야 한다.
1-b. **Stage 3(LLM 판정) 전량 실패 — 해결 (2026-09-19)**. 모델이 `"주어진 정보를
   분석하겠습니다."` 같은 서두를 먼저 쓰고 `max_tokens=50` 에서 잘려 JSON 이 아예
   나오지 않았다. 2026-09-19 실행에서 24번 호출돼 24번 전부 파싱 실패했고, 그 기사들이
   모두 "새 사건"으로 떨어져 중복이 됐다. assistant 턴을 `{"match":` 로 prefill 해서
   고쳤다 — 같은 실행 조건에서 19/19 매칭 성공, 실패 0.
   **NEW 비율 67% → 42%.** 다만 그 전에 이미 만들어진 중복은 그대로 남아 있다.
10. **score_snapshots 스키마 드리프트 — 마이그레이션 025 적용 필요**.
   테이블이 v1.0 구조(`blue_negative`/`blue_net`)인데 크롤러와 앱은 `blue_score`/`blue_count`
   를 쓴다. 그래서 크롤러가 매 실행 끝에 PGRST204 로 죽었고 **`score_snapshots` 행이 0개**다.
   일별 추이가 한 번도 쌓이지 않았다. `supabase/025-score-snapshots-v11.sql` 실행하면 된다.
11. ~~**EVENT_MATCH_THRESHOLD(0.85)가 높다**~~ — **2026-09-19 해결: 0.78 로 인하**.
   2026-09-19 실측: 거의 같은 문장인 `"김승원 법무장관 후보자 자진 사퇴"` 와
   `"김승원 법무부 장관 후보자 자진 사퇴"` 가 **0.774** 다. 같은 인물·범주·7일 이내
   쌍 73건 중 0.85 를 넘는 건 4건뿐이었고, 0.70~0.85 대역에 명백한 중복이 다수 있다.
   임계가 높으면 Stage 2 가 회색지대로 떠넘기고 Stage 3(LLM) 비용이 는다.
   낮추면 1심/항소심처럼 "같은 사건의 다른 국면"까지 합쳐질 수 있어 제품 판단이 필요하다.
   국면을 잇는 건 사안(storyline) 계층의 일이므로 사건 단위는 과감히 합친다.
   사안 발견 품질이 사건 중복에 직접 영향을 받는 것도 확인됐다(2건짜리 사안 상당수가
   서로 중복이었다).
...
6. ~~**유사 사례 무작위**~~ — **해결 (2026-09-19)**. `match_similar_events` 가 유사도로
   `NaN` 을 돌려주던 버그였다(영벡터 → pgvector `<=>` 가 NaN → Postgres 가 NaN 을 최댓값으로
   취급 → 임계값 통과·정렬 1위). `supabase/022-embedding-nan-guard.sql` 로 증상을 막고,
   `backfill_embeddings.py` 로 임베딩 67건을 다시 채워 실제로 복구했다.
   검증: 탄핵 인용 → 12·3 계엄 탄핵(0.848) 1위, 특별사면 사건끼리 0.77~0.82 로 묶인다.
